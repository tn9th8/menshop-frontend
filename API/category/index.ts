...
export async function getCategoriesForClient(): Promise<ICategory[]> {
  try {
    const response = await api.get('api/v1/client/categories/tree')
    return response?.data?.data
  } catch (error) {
    handleError(error as Error, 'API/category', 'getCategoriesForClient')
